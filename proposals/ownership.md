...
This is a restriction that resembles what `[UnscopedRef]` can express for some existing `ref`-based APIs: the attribute can cause a returned `Span<T>` to be treated as borrowing from the receiver, so a span obtained from a local receiver cannot be returned. However, `[UnscopedRef]` is not the right model for this API. The intent here is not to widen the lifetime of `this`, but to say that the returned `Span<T>` is borrowed from the `RentedArray` and must not outlive it. Making this precise requires the borrow/escape-analysis rules described earlier in this proposal to be filled in; until then, this should be read as a statement of intent rather than as something expressible with today's C# syntax.
...
void Write(string s)
{
    Span<byte> utf8 = s.Length < MAXSTACK
        ? stackalloc byte[MAXSTACK * 2]
        : Array.Empty<byte>();
...
This is the classic, "stackalloc a span if small or use a rented array if large". In both cases the usage is local and simple. Unlike the existing use of `ArrayPool` there is no risk of forgetting to free, or double free. In fact, free is not callable by the renter. Instead, the Rented array is automatically dropped at the end of its scope, and a custom `Drop` implementation returns the array to the pool when dropped. Use of the array is limited to the `Span`, but this is sufficient for the task. Notably, the lifetime of the `Span` is tied to the Rented array as well, so it cannot outlive it.
